...
| [checks/validate-plan-delegation.ts](checks/validate-plan-delegation.ts) | A plan that delegates live work to another plan **points** at it and does not **restate** its state: a block in an active plan's `## Next actions` linking another *active* plan is capped at 10 non-blank lines / 100 words (E-009, ring [0058](../docs/rings/0058-a-plan-points-not-restates.md)). `## Progress log` is exempt — dated history, whose size is its job — as are completed plans and links to them; silent when nothing delegates | LAW-5 |
